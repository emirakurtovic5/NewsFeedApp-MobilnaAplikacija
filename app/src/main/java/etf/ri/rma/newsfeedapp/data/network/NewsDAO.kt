...
class NewsDAO(private val api: NewsApiService) {
    ...
    val predefinedStories = listOf(
        NewsItem(
            ...
            imageTags = arrayListOf("general", "politics")
        ),
        NewsItem(
            ...
            imageUrl = "https://media-cldnry.s-nbcnews.com/image/upload/t_nbcnews-fp-1200-630,f_auto,q_auto:best/rockcms/2025-05/250502-npr-mb-0756-b5e7de.jpg",
            category = "politics",
            isFeatured = false,
            source = "nbcnews.com",
        ...
        NewsItem(
            ...
            imageTags = arrayListOf("general", "business")
        ),
        NewsItem(
                ...
                try {
                    ...
                    allStories.forEachIndexed { idx, item ->
                        if (item.category.equals(category, ignoreCase = true)) {
                            allStories[idx] = item.copy(isFeatured = false)
                        }
                    }
                    val featuredList = mutableListOf<NewsItem>()
                    for (newItem in newlyFetched) {
                        val existingIndex = allStories.indexOfFirst { it.uuid == newItem.uuid }
                        if (existingIndex != -1) {
                ...
                withContext(Dispatchers.IO) {
                    ...
                    mutex.withLock {
                        similarStories.forEach { newStory ->
                            if (allStories.none { it.uuid == newStory.uuid }) {
